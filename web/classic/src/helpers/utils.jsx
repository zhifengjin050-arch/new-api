...
// -------------------------------
// 缓存价格 fallback 计算（当数据库无 cache_ratio 时使用）
const getFallbackCacheRatios = (modelName) => {
  if (!modelName) return { cacheRead: null, cacheCreate: null };
  const name = modelName.toLowerCase();

  // Claude 3 系列: 缓存读取 = 输入 * 0.10, 缓存创建 = 输入 * 1.25
  if (name.includes('claude-3')) {
    return { cacheRead: 0.1, cacheCreate: 1.25 };
  }

  // DeepSeek 系列: 缓存读取 = 输入 * 0.25, 缓存创建 = 输入 * 1.0
  if (name.includes('deepseek')) {
    return { cacheRead: 0.25, cacheCreate: 1.0 };
  }

  return { cacheRead: null, cacheCreate: null };
};
...
export const calculateModelPrice = ({
  ...
  if (record.quota_type === 0) {
    ...
    // 缓存价格 fallback：当数据库无 cache_ratio 时，根据模型名推断
    const fallbackCache = getFallbackCacheRatios(record.model_name);
    const cachePrice = hasRatioValue(record.cache_ratio)
      ? formatTokenPrice(inputRatioPriceUSD * Number(record.cache_ratio))
      : fallbackCache.cacheRead !== null
        ? formatTokenPrice(inputRatioPriceUSD * fallbackCache.cacheRead)
        : null;
    const createCachePrice = hasRatioValue(record.create_cache_ratio)
      ...
      : fallbackCache.cacheCreate !== null
        ? formatTokenPrice(inputRatioPriceUSD * fallbackCache.cacheCreate)
        : null;
