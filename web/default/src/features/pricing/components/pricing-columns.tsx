  ...
  return [
      ...
      cell: ({ row }) => {
        ...
        if (isTokenBased) {
          const inputPrice = formatPrice(
            model, 'input', tokenUnit, showRechargePrice, priceRate, usdExchangeRate
          )
          const outputPrice = formatPrice(
            model, 'output', tokenUnit, showRechargePrice, priceRate, usdExchangeRate
          )
          const showCache = hasCachePricing(model)

          return (
            <div className='max-w-full min-w-0 space-y-0.5'>
              <div className='flex items-center justify-between gap-2 text-xs'>
                <span className='text-muted-foreground shrink-0'>{t('Input')}</span>
                <span className='font-mono font-semibold tabular-nums text-right'>
                  {stripTrailingZeros(inputPrice)}
                  <span className='text-muted-foreground/50 font-normal ml-0.5'>/{tokenUnitLabel}</span>
                </span>
              </div>
              <div className='flex items-center justify-between gap-2 text-xs'>
                <span className='text-muted-foreground shrink-0'>{t('Output')}</span>
                <span className='font-mono font-semibold tabular-nums text-right'>
                  {stripTrailingZeros(outputPrice)}
                  <span className='text-muted-foreground/50 font-normal ml-0.5'>/{tokenUnitLabel}</span>
                </span>
              </div>
              {showCache && (
                <>
                  <div className='flex items-center justify-between gap-2 text-xs'>
                    <span className='text-muted-foreground shrink-0'>{t('Cache Read')}</span>
                    <span className='font-mono font-semibold tabular-nums text-right'>
                      {stripTrailingZeros(
                        formatPriceWithFallback(model, 'cache', tokenUnit, showRechargePrice, priceRate, usdExchangeRate)
                      )}
                      <span className='text-muted-foreground/50 font-normal ml-0.5'>/{tokenUnitLabel}</span>
                    </span>
                  </div>
                  <div className='flex items-center justify-between gap-2 text-xs'>
                    <span className='text-muted-foreground shrink-0'>{t('Cache create price')}</span>
                    <span className='font-mono font-semibold tabular-nums text-right'>
                      {stripTrailingZeros(
                        formatPriceWithFallback(model, 'create_cache', tokenUnit, showRechargePrice, priceRate, usdExchangeRate)
                      )}
                      <span className='text-muted-foreground/50 font-normal ml-0.5'>/{tokenUnitLabel}</span>
                    </span>
                  </div>
    ...
    // Cached price column (Vercel AI Gateway style)
    {
      ...
      cell: ({ row }) => {
        ...
        if (!isTokenBased || !hasCachePricing(model)) {
          return <span className='text-muted-foreground/30 text-xs'>—</span>
        }
