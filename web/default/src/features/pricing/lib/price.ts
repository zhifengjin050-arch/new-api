...
export function getFallbackCacheRatios(modelName: string): {
...
export function hasCachePricing(model: PricingModel): boolean {
  if (model.cache_ratio != null || model.create_cache_ratio != null) {
    return true
  }
  const fallback = getFallbackCacheRatios(model.model_name || '')
  return fallback.cacheRead != null || fallback.cacheCreate != null
}
...
export function formatPriceWithFallback(
  ...
  if (Number.isNaN(priceInUSD)) {
    ...
    if (type === 'cache' && fallback.cacheRead != null) {
      priceInUSD = base * fallback.cacheRead
    } else if (type === 'create_cache' && fallback.cacheCreate != null) {
      priceInUSD = base * fallback.cacheCreate
    } else {
      return '-'
    }
  }
